...
// ДЗ №4
func TestGetHandlerExtFilter(t *testing.T) {
	// Создаем запрос с указанием нашего хендлера. Так как мы тестируем GET-эндпоинт
	// то нам не нужно передавать тело, поэтому третьим аргументом передаем nil
	req, err := http.NewRequest("GET", "/?ext=book.png", nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	// Проверяем тело ответа
	expected := `[{"Name":"book.png","Size":28673,"Mode":438,"ModTime":"2010-11-14T06:54:51+03:00","IsDir":false}]`
